...
const getTask = (buildType, options) => {
	const pipeline = pipelines[buildType];

	if (pipeline === void 0) {
		console.warn(`There is no pipeline configured for ${buildType}`);
	...
	task.displayName = options.name || (buildType + ':' + (options.file || util.uniqueId()));

	return task;
};

const getAllTasks = buildersConfig => {
	const tasks = [];

	Object.keys(buildersConfig).forEach(buildType => {
		buildersConfig[buildType].forEach(options => {
			const task = getTask(buildType, options);
